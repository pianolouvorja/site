...
// Stub h3 functions globally (only affects this test file's unit environment)
const globalReadBody = vi.fn(async () => mockReadBody)
const globalCreateError = vi.fn((opts: Record<string, unknown>) => {
...
// readBody and createError are provided globally via stubs above
...
describe('handleSubscribe', () => {
  beforeEach(() => {
    mockFetch.mockReset()
    globalCreateError.mockClear()
    globalReadBody.mockClear()
    globalReadBody.mockImplementation(async () => mockReadBody)
  ...
  it('throws service-unavailable when API key is not configured', async () => {
    ...
    expect(globalCreateError).toHaveBeenCalledWith(
      expect.objectContaining({
        statusCode: 503,
